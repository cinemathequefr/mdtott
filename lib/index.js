...
const cudm = require("./cudm");
const replace_ansi_by_entities = require("./replace_ansi_by_entities");
...
async function main(path) {
  ...
  data = replace_ansi_by_entities(data);
...
${data}`;
  ...
  try {
    let outputPath = path.replace(/(.+).md/, "$1.html");
